			...
			try
				{
					while (true)
					{
						...
						if  ((buffer.toString().indexOf("OK\r") >= 0) || 
								((buffer.toString().indexOf("ERROR") >= 0) && (buffer.toString().indexOf("\r") > buffer.toString().indexOf("ERROR")))) break;
						if (buffer.toString().indexOf("CPIN") >= 0)
						{
							Thread.sleep(100);
							while (dataAvailable()) buffer.append((char) inStream.read());
							break;
						}
					}
					retry = RETRIES;
